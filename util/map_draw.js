...
export const map_draw = {
    ...
    draw: (ctx, map) => {
        if (map.shapes != undefined) {
            ...
            map_draw.shapes_on_screen = map.shapes.filter((s) => s.computed?.on_screen).sort((a, b) => {
                if (a.id === m_ui.properties_selected.id)
                    return 1;
                if (b.id === m_ui.properties_selected.id)
                    return -1;
                ...
                if (o2.sensor && !o1.sensor)
                    return -1;
                if (o1.is_spawner && !o2.is_spawner)
                    return 1;
                if (o2.is_spawner && !o1.is_spawner)
                    return -1;
    ...
    draw_shape: (ctx, shape, shadow = false) => {
    ...
    draw_shape_ui: (ctx, shape, shadow = false) => {
        if (shape.computed?.screen_vertices == undefined || shape.computed.screen_vertices.length <= 0 || shape.computed.shadow_vertices == undefined)
            return;
            ...
            if (m_ui.mouse.drag_target[0].id === id_) {
                ...
                if (mouse.drag_vector_old[0] !== false && !m_ui.circle_menu.active) { // if the user is actually dragging the mouse
                    if (o.new) {
                        ...
                        const change = vector.div(mouse.drag_change[0], camera.scale * camera.zscale(o.vertex.z - camera.look_z));
