...
export const math = {
  ...
  pixel_roughly_equal: (a: number, b: number): boolean => {
  ...
  round_rand: (value: number): number => {
    const i = Math.floor(value), f = value - i;
    if (math.equal(value, i)) return i;
    else if (math.rand() < f) return i + 1;
    else return i;
  },
  ...
  line_segment_intersection_old: (line1: [vector, vector], line2: [vector, vector]): boolean => {
  ...
  line_segment_intersection: (line1: [vector, vector], line2: [vector, vector]): boolean => {
    const [ p, p_ ] = line1, [ q, q_ ] = line2;
    const s10_x = p_.x - p.x, s10_y = p_.y - p.y,
          s32_x = q_.x - q.x, s32_y = q_.y - q.y;
  ...
  is_line_intersecting_polygons: (v1: vector, v2: vector, polygons: vector[][]): false | [vector[], number] => {
    for (const polygon of polygons) {
      const l = polygon.length;
      for (let i = 0; i < l - 1; i++) {
        if (math.line_segment_intersection([v1, v2], [polygon[i], polygon[i + 1]])) return [polygon, i];
      }
    }
    return false;
  },
