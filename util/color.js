...
export const THEMES_UID_MAX = Object.values(THEMES).map((t) => t.uid).reduce((id1, id2) => Math.max(id1, id2)); // i don't normally put everything in 1 line lol
export const current_theme = clone_object(THEMES.default);
export const color2hex = (c) => {
    return "" + (current_theme[c] ?? THEMES.default[c] ?? c);
};
export const color2hex_map = (c, theme_string) => {
    if (!(theme_string in THEMES)) {
        theme_string = make_rooms[theme_string]?.theme;
    }
    const t = (theme_string ? (THEMES[theme_string] ?? THEMES.default) : current_theme);
    const tc = t[c] ?? THEMES.default[c];
    if (tc)
        return "" + tc;
    else
        return c;
