...
export const camera = {
  ...
  screen2world_target: function(screen_vector: vector, scale?: number): vector {
    return vector.add(vector.mult(screen_vector, 1 / (scale ?? this.scale_target)), this.position_target);
  },
  world2screen_target: function(world_vector: vector, scale?: number): vector {
  ...
  zscale: function(z: number, is_width: boolean = false): number {
    if (math.equal(this.z, z)) return is_width ? config.graphics.linewidth_max_mult : 1 / ZEPSILON;
    let z_ = (this.z - this.look_z) / (this.z - z); // replaced this.z / this.scale
    if (z_ <= 0) z_ = is_width ? config.graphics.linewidth_max_mult : 1 / ZEPSILON;
    return z_;
  },
  ...
  tick: function(dt: number) {
    ...
    if (this.lerp_factor >= 1 - math.epsilon) {
      this.lerp_factor--;
      if (this.lerp_factor <= 0) this.lerp_factor = config.graphics.camera_smoothness;
