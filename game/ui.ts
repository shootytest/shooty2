...
import { Thing, thing_speech } from "./thing.js";
...
export const ui = {
  ...
  toggle_pause: () => {
    ...
    else if (player.shapes_mode) ui.toggle_shapes();
    else ui.pause_f();
  },
  toggle_map: () => {
    if (player.map_mode) {
  ...
  draw_pause_menu: function() {
    if (player.some_mode) { // draw mode-switcher on top
      ...
      if (hovering) ui.click.new(ui.toggle_left);
      ...
      if (hovering) ui.click.new(ui.toggle_right);
      ctx.fillStyle = color_mix(current_theme.dark, color.black, hovering ? 0.3 : 0.5);
      ctx.beginPath();
      ctx.circle(x, y, size * 1.7);
      ctx.fill();
      ...
      for (let i = 0; i < length; i++) {
        ...
        ctx.svg_v(o.icon as keyof typeof SVG, v, r);
        ctx.beginPath();
  ...
  shapey: {
    ...
    coin_attractor: {
      ...
      on_fn: (n: number) => {
        ...
        if (n >= 2) config.game.coin_drop_mult *= [1, 1, 1.1, 1.2, 1.4][n];
      },
      off_fn: (n: number) => {
  ...
  } as { [key: string]: { title: string, description: string[], base?: boolean, area?: number[], on_fn?: (n: number) => void, off_fn?: (n: number) => void } },
  ...
  text: {
    circle: {
      draw: function(s: string, x: number, y: number, size: number, align: CanvasTextAlign = "center", kern: number = 0.15) {
        if (align === "center") x -= size * (1 + kern) * (s.length - 1) / 2;
        else if (align === "left") x += size / 2;
        else if (align === "right") x -= size * (1 + kern) * (s.length - 1) + size / 2;
        for (const w of s.split(" ")) {
          ...
          ctx.stroke();
          ctx.clip();
          if (Object.hasOwn(this, w)) (this as any)[w]?.(x, y, size / 2);
          else console.error(`[ui/text/circly/draw] invalid word in string ${s}: ${w}`);
          ctx.ctx.restore();
          x += size * (1 + kern);
        }
      },
    ...
    circulatin: {
      draw: function(s: string, x: number, y: number, size: number, align: CanvasTextAlign = "center", kern: number = 0.15) {
        ...
        for (let i = 0; i < s.length; i++) {
          ...
          ctx.translate(x, y);
          ctx.rotate(a);
          if (Object.hasOwn(this, c)) (this as any)[c]?.(0, 0, size / 2);
          else console.error(`[ui/text/circly/draw] invalid character in string ${s}: ${c}`);
          ctx.ctx.restore();
          x += size * (1 + kern);
        }
      },
  ...
  draw_speech: function() {
    ...
    for (const t of Shape.draw_things) {
      if (!t.is_speech_active) continue;
      if (t.speech.text.length > t.speech.display.length) t.speech.display.length += 1;
      this.draw_speech_bubble(t, t.speech);
    }
  },

  draw_speech_bubble: function(shape: Shape | Thing, speech: thing_speech) {
    if (shape instanceof Thing) shape = shape.shapes[0];
  ...
  particles: {
    ...
    } as { [key: string]: { n: number[], fn: () => Particle }},
    tick: function() {
