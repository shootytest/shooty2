...
export class Shape {
  ...
  static filter(screen_aabb: AABB3): Shape[] {
    ...
    for (const s of Shape.shapes) {
      ...
      if (s.z < camera.look_z - 1 || s.z >= camera.look_z + 1) continue; // cullingz
  ...
  init_computed() {
    ...
    const aabb = vector.make_aabb(calc_vertices),
      ...
      mean = this.closed_loop ? vector3.create2(Vertices.centre(calc_vertices), vector3.meanz(calc_vertices)) : vector3.mean(calc_vertices); // don't use mean...
