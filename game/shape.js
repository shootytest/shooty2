...
import { clone_object, make_shoot, multiply_and_override_object, override_object, shallow_clone_array } from "./make.js";
...
export class Shape {
    ...
    static from_map(thing, o) {
        ...
        if (thing.options.translucent)
            s.translucent = thing.options.translucent;
        if (thing.options.translucent_color)
            s.translucent_color = thing.options.translucent_color;
        // handle vertices
    ...
    ;
    static from_make(thing, o, ignore_shoot = false) {
        let s;
    ...
    ;
    static arc(thing, radius, arc_start, arc_end, offset) {
    ...
    ;
    static polyline(thing, vs, closed_loop, z = 0) {
        const s = new Shape(thing);
    ...
    static filter(screen_aabb) {
        ...
        for (let z = -1; z <= 1; z += 0.1) {
            ...
            if (math.equal(z, 0))
                Shape.memo_aabb3[z_string.substring(1)] = vector3.aabb_scale(screen_aabb, vector3.create(z_scale, z_scale, 1));
        }
    ...
    static draw(z) {
        for (const s of Shape.draw_shapes) {
            if (z != undefined && !math.equal(s.draw_z, z))
                continue;
            s.draw_all();
        }
        ctx.globalAlpha = 1;
    ...
    static enemy_block_vertices = []; // list of walls that block enemies' line of sight
    ...
    static calc_other_vertices() {
        ...
        for (const s of Shape.draw_shapes) {
            ...
            if (s.translucent_color)
                key += "|" + s.translucent_color;
            if (!result[key])
                result[key] = [];
            result[key]?.push(vs);
        }
    ...
    init_computed() {
        ...
        if (this.activate_scale) {
            if (this.vertices.length > 2)
                vertices = vector3.scale_list(this.vertices, this.scale);
    ...
    real_vertices(round = true) {
        ...
        if (round)
            return vector3.round_list(vs, 1, 0.001);
        else
            return vs;
    }
    add(thing) {
    ...
    draw_highlight() {
        if (!this.options.highlight || this.options.highlight < 0)
            return;
