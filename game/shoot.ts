...
export class Shoot {
  ...
  get ratio() {
    if (this.time >= (this.stats.reload ?? 0)) return 1;
    return math.bound((this.time / (this.stats.reload ?? 1) - (this.stats.delay ?? 0) + 1) % 1, 0, 1);
  }

  init_mods(exclude_objects: bullet_mod[] = []) {
    if (this.stats.mods) for (const m of shallow_clone_array(this.stats.mods)) {
      if (exclude_objects.includes(m)) continue;
      const modded_stats = clone_object(this.stats) as shoot_stats;
  ...
  add_mod(mod: bullet_mod) {
    if (!this.stats.mods) this.stats.mods = [];
    let old_mods = shallow_clone_array(this.stats.mods ?? []);
    this.stats.mods.push(mod);
    this.init_mods(old_mods);
  }

  add_mod_key(key: string) {
    if (!make_shoot_mods[key]) return;
    this.add_mod(make_shoot_mods[key]);
  }

  remove_mod_key(id: string) {
    if (!this.stats.mods) return;
    for (const m of shallow_clone_array(this.stats.mods)) {
      if (m.id === id) this.stats.mods.remove(m);
    }
  }
  ...
  mult_stats(new_stats: shoot_stats) {
    let old_mods = shallow_clone_array(this.stats.mods ?? []);
  ...
  tick(dt: number) {
    ...
    if (this.stats.always_shoot) this.shoot();
  }
  ...
  shoot_bullet() {
    ...
    if (the_stats.mods) for (const m of shallow_clone_array(the_stats.mods)) {
      const o = m.calc;
      if (!o) continue;
      o.number++;
      if (o.number >= (o.period ?? 1)) {
        if (o.chance != undefined && math.rand() > o.chance) {
          ...
          if (o.chance < 0) o.chance = m.chance;
          continue;
        }
        the_stats = o.stats;
        o.number = 0;
    ...
    if (S.random_angular_speed) angular_speed += math.rand(-S.random_angular_speed, S.random_angular_speed);
    ...
    if (S.random_size) size += math.rand(-S.random_size, S.random_size);
    ...
    if (S.detect_range_mult != undefined && bullet.options.detect_range) bullet.options.detect_range *= S.detect_range_mult;
