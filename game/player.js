...
import { make, override_object, shallow_clone_array } from "./make.js";
...
export class Player extends Thing {
    ...
    on_floor = 0; // 2 = safe, 1 = not safe, 0 = fallllling
    floor_z = 0;
    is_safe = true;
    is_on_checkpoint = false;
    map_room = "";
    ...
    constructor() {
        ...
        if (this.health)
            this.health.display = 0; // start smooth animation from zero instead of full health
        ...
        if (!this.paused) {
            ...
            for (const s of Shape.floor_shapes) {
                if (s.computed && math.is_circle_in_polygon(this.position, this.radius, s.computed.vertices)) {
                    if (s.z + math.epsilon < on_floor_z)
                        break; // gone below the first floor level that the player is above
                    floor_z = s.z;
                    if (!on_floor)
                    ...
                    if (on_floor) {
                        ...
                        if (!touching_floors.includes(s.thing))
                            touching_floors.push(s.thing);
                        safe_floor = safe_floor && (s.options.safe_floor ?? s.thing.options.safe_floor ?? false);
                        if (!checkpoint_floor)
                            checkpoint_floor = Boolean(s.thing.options.checkpoint);
                    }
                }
            }
            ...
            // map
            if (this.is_safe && on_floor && grounded) {
            ...
            if (Thing.time >= this.autosave_time) { // run every autosave interval
                ...
                if (this.is_safe && (safe_floor && on_floor) && grounded)
                    this.save(); // only save while safe
    ...
    camera_zs() {
        if (this.inventory_mode || this.shapes_mode)
            return [1, 0];
    ...
    load(o) {
        ...
        this.target.vz = 0;
        if (o.checkpoint)
            this.checkpoint = vector3.clone(o.checkpoint);
    ...
    collect(o) {
        if (o.restore_all_health)
            this.health?.heal_all();
        else if (o.restore_health)
            this.health?.heal(o.restore_health);
        if (o.gun) {
    ...
    set_checkpoint_to_thing(thing, shape_position = false) {
    ...
    change_room(room_id, force = false) {
        if (!room_id || (!force && this.room_id === room_id))
            return;
    ...
    load_room(room_id) {
        ...
        for (const id of MAP.computed?.room_map[room_id] ?? []) {
            ...
            if (s && !s.options.map_parent && !this.lookup(s.id))
    ...
    unload_room_visitors(room_id) {
        for (const spawner of shallow_clone_array(Spawner.spawners_rooms[room_id] ?? [])) {
            if (spawner.original_room_id !== room_id)
                spawner.remove();
        }
        for (const thing of shallow_clone_array(Thing.things_rooms[room_id] ?? [])) {
            if (thing.original_room_id !== room_id)
                thing.remove();
        }
    }
    reload_room(room_id) {
    ...
    activate_inventory() {
        ...
        const wall = this.lookup("home inventory wall");
        this.temp_border = vector.add_list(wall.shapes[0].vertices, wall.position);
        for (const b of wall?.body?.walls ?? [])
        ...
        for (const s of thing_ids) {
            ...
            t.angle = math.randangle();
            this.temp_things.push(t);
            t.create_body();
            t.set_velocity(v);
            if (Enemy.cumulative_ids[s] == undefined)
                Enemy.cumulative_ids[s] = 1;
            t.create_id(s + " #" + Enemy.cumulative_ids[s]++);
            if (t.body)
    ...
    deactivate_shapes() {
        ...
        for (const b of this.lookup("home shapestore wall")?.body?.walls ?? [])
            Composite.remove(world, b);
        Composite.remove(world, ui.mouse.constraint);
        camera.lerp_factor = 1;
        this.temp_things = [];
