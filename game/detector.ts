...
import { clone_object, maketype_behaviour } from "./make.js";
...
export const shared_object: { [key: string]: any } = {};

export const detector = {
  // magic!
  meow_check: function(pair: Matter.Pair, ba: any, bb: any, a: Thing, b: Thing, type: "start" | "middle" | "end") {
    if (ba.z == undefined) return;
    ...
    if (type === "start") {
      ...
      if (bb[sc] == undefined) {
        ...
        bb[sc].push(ba.z);
        bb[sp].push(pair);
        const min_z = Number((bb[sc] as number[]).reduce((z1, z2) => Math.min(z1, z2)).toFixed(3));
        const max_z = Number((bb[sc] as number[]).reduce((z1, z2) => Math.max(z1, z2)).toFixed(3));
        if (bb[sp].length === 2) bb[sp][0].z_range = [min_z, max_z];
        (pair as any).z_range = [min_z, max_z];
      ...
      if ((pair as any).z_range == undefined) return;
      const [min_z, max_z] = (pair as any).z_range as [number, number];
      if ((pair as any).z_diff) {
        ...
        if (min_z <= b.z && max_z > b.z) { // hits!
          ...
          for (const p of bb[sp]) {
            p.isSensor = false;
            p.z_diff = false;
            detector.collision_start(p, p.bodyA.parent.thing as Thing, p.bodyB.parent.thing as Thing, true);
            detector.collision_start(p, p.bodyB.parent.thing as Thing, p.bodyA.parent.thing as Thing, true);
          }
        }
      } else {
        ...
        if (!(min_z <= b.z && max_z > b.z)) {
          for (const p of bb[sp]) {
            p.isSensor = true;
            p.z_diff = true;
            detector.collision_end(p, p.bodyA.parent.thing as Thing, p.bodyB.parent.thing as Thing, true);
            detector.collision_end(p, p.bodyB.parent.thing as Thing, p.bodyA.parent.thing as Thing, true);
          }
        }
      }
    } else if (type === "end") {
      // collision end
      if (bb[sc] != undefined) bb[sc].remove(ba.z);
      if (bb[sp] != undefined) bb[sp].remove(pair);
      if (bb[sc].length >= 1) {
        const min_z = bb[sc].length >= 2 ? Number((bb[sc] as number[]).reduce((z1, z2) => Math.min(z1, z2)).toFixed(3)) : bb[sc][0];
        const max_z = bb[sc].length >= 2 ? Number((bb[sc] as number[]).reduce((z1, z2) => Math.max(z1, z2)).toFixed(3)) : bb[sc][0];
        for (const p of bb[sp]) {
  ...
  init: function() {
    Events.on(engine, "collisionStart", function(event) {
      for (const pair of event.pairs) {
        ...
        detector.meow_check(pair, ba as any, bb as any, a, b, "start");
        detector.meow_check(pair, bb as any, ba as any, b, a, "start");
    ...
    Events.on(engine, "collisionActive", function(event) {
      for (const pair of event.pairs) {
        const ba = pair.bodyA, bb = pair.bodyB;
        const a = ((ba.parent as any).thing as Thing), b = ((bb.parent as any).thing as Thing);
        detector.meow_check(pair, ba as any, bb as any, a, b, "middle");
        detector.meow_check(pair, bb as any, ba as any, b, a, "middle");
    ...
    Events.on(engine, "collisionEnd", function(event) {
      for (const pair of event.pairs) {
        const ba = pair.bodyA, bb = pair.bodyB;
        const a = ((ba.parent as any).thing as Thing), b = ((bb.parent as any).thing as Thing);
        detector.meow_check(pair, ba as any, bb as any, a, b, "end");
        detector.meow_check(pair, bb as any, ba as any, b, a, "end");
        detector.collision_end(pair, a, b);
        detector.collision_end(pair, b, a);
      }
    });
  },
  collision_start: (pair: Matter.Pair, a: Thing, b: Thing, suddenlyz: boolean = false) => {
  ...
  sensor_during_fns: {
    ...
    ["station tracks"]: (thing) => {
      ...
      if (math.equal(player.z, 0) && !thing.lookup("train sensor").is_touching_player) player.push_by(vector.create(train_position === 11.5 ? 10 : -10, 0));
    },
    set_train: (thing) => {
    ...
    ["station tutorial floor"]: (thing, dt) => {
      if (player.is_safe) {
        if (!save.check_switch("train")) save.set_switch("train", 11);
    ...
    ["station streets floor 1"]: (thing, dt) => { if (player.is_safe) detector.sensor_during_fns.set_train(thing, dt); },
    ["station streets floor 2"]: (thing, dt) => { if (player.is_safe) detector.sensor_during_fns.set_train(thing, dt); },
    ["station streets floor 3"]: (thing, dt) => { if (player.is_safe) detector.sensor_during_fns.set_train(thing, dt); },
    ...
    ["streets room 7 sensor 1"]: (_thing, _dt) => {
      ...
      if (!s.is_done) player.camera_target_target = s.vertices[0];
    },
    ["train sensor"]: (_thing, dt) => {
      ...
      if (train_time > 2.1 * config.seconds || train.object.crashed) {
        if (train.object.crashed) {
          if (train_time < 2 * config.seconds) player.object.train_time += 2 * config.seconds;
  ...
  before_spawn_fns: {
    ["enemy_streets_camera_small"]: (thing) => {
      if (save.check_switch("jump")) {
        ...
        if (b) (b as maketype_behaviour).shoot_mode = "normal";
    ...
    ["enemy_streets_turret_1"]: (thing) => {
      const centre = thing.position;
      thing.object.ns = [] as number[];
      thing.object.children = [] as Thing[];
      for (let i = 0; i < 6; i++) {
        const angle = 60 * i;
        const t = new Thing();
        t.parent = thing;
        ...
        t.create_body();
        const range: [number, number] = [angle, angle + 60];
        thing.options.repel_angles?.push(range);
        t.object.range = range;
        thing.object.children.push(t);
      }
      thing.remove_fn = () => {
        for (const c of thing.object.children ?? []) c.remove();
      };
    },
  } as {
  ...
  } as { [map_shape_id: string]: (map_shape: map_shape_type) => void },
  ...
  tick_fns: {
      ...
      if (show) {
        ...
        if (!thing.object.block_done) {
          ...
          if (block) {
            ...
            for (const s of block.shapes) s.seethrough = true;
          }
        }
      } else {
    ...
    ["streets room 7 door"]: (door) => {
      const s = Spawner.spawners_lookup["streets room 7 waves"];
      if (s) do_door(door, !s.is_done && s.wave_started);
    },
    ...
    ["streets room 7 floor moving up"]: (floor) => {
      ...
      if (floor.object.down > 0) {
        if (floor.object.down > Thing.time) return;
        if (floor.position.y > floor.original_position.y) {
          floor.object.down = -(Thing.time + config.seconds * 0.5);
          floor.set_velocity(vector.create());
        } else floor.set_velocity(vector.create(-1, 3));
    ...
    ["streets room 7 floor moving down"]: (floor) => {
      ...
      if (floor.object.down > 0) {
        ...
        if (floor.position.y > floor.original_position.y + 875) {
          floor.object.down = -(Thing.time + config.seconds * 0.5);
          floor.set_velocity(vector.create());
        } else floor.set_velocity(vector.create(-1.5, 4.5));
      } else if (floor.object.down < 0) {
        if (-floor.object.down > Thing.time) return;
        if (floor.position.y < floor.original_position.y) {
          floor.object.down = Thing.time + config.seconds * 0.5;
...
const map_switch_door = (door: Thing, switch_ids: string | boolean | (string | boolean)[], path_id: string) => {
  let step = door.object.step ?? 1;
  if (door.object.step == undefined) {
  ...
  if (!Array.isArray(switch_ids)) switch_ids = [switch_ids];
  if (step > switch_ids.length) return;
...
const map_copy_position = (thing: Thing, other: string | Thing) => {
  if (typeof other === "string") other = thing.lookup(other);
  if (other?.position) thing.position = vector.clone(other.position);
};