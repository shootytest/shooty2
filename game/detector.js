...
export const detector = {
    ...
    tick_fns: {
        ...
        ["streets room 3.1 floor 2"]: (floor) => {
            if (floor.object.down && floor.position.y > floor.original_position.y + 450)
                floor.object.down = false;
            else if (!floor.object.down && floor.position.y < floor.original_position.y)
                floor.object.down = true;
    ...
    behaviour_fns: {
        ["npc_test idle start"]: (thing) => {
            if (!thing.is_speech_active)
                thing.say("i'm bored");
        },
        ["npc_test idle"]: (thing) => {
            if (!thing.is_speech_active)
                thing.say("grrr i'm board");
        },
...
const do_floor = (floor, conditions, path_id) => {
