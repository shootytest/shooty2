...
export const ui = {
    ...
    mouse: {
        ...
        original_angles: [],
        get selected_id() {
            return (ui.mouse.selected?.object.shapey_id ?? ui.mouse.hovered?.object.shapey_id);
        },
        init: () => {
            ...
            Events.on(ui.mouse.constraint, "mousedown", (e) => {
                if (ui.mouse.constraint.body)
                    return;
                const old = ui.mouse.selected;
                if (old?.shapes[0])
                    old.shapes[0].options.highlight = 0;
    ...
    map: {
        ...
        tick: () => {
            if (ui.map.time > config.graphics.map_fade_time)
                ui.map.opacity = player.map_mode ? 1 : 0;
            else {
    ...
    shapey: {
        friendly: {
            ...
            off_fn: (_n) => {
                ...
                player.object.old_shape_opacity?.forEach((o, i) => {
                    if (i > 0)
                        player.shapes[i].opacity = o;
                });
            },
        },
        triangle_speed: {
        ...
        for (const [k, o] of Object.entries(ui.shapey)) {
            ...
            for (let n = 1; n <= o.description.length; n++) {
                ...
                for (const s of ss) {
                    ...
                    if (s.type === "circle")
                        area += mult * math.circle_area(s.radius ?? 0);
                    else if (s.type === "polygon")
                        area += mult * math.polygon_area(s.sides ?? 16, s.radius ?? 0);
                    else if (s.type === "polyline")
                        area += mult * math.vertices_area(s.vs ?? []);
                    // else area += mult * 0;
                    if (!o.base)
                        break;
                }
